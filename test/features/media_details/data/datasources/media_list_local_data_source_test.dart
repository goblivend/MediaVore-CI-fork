...
void main() {
  ...
  group('MediaListLocalDataSource - Seen Items', () {
    ...
    test(
      'should allow multiple seen entries for the same item (Multi-viewing)',
      () async {
        final firstDate = DateTime(2023, 10, 1);
        final secondDate = DateTime(2023, 10, 2);

        await dataSource.markAsSeen(
          SeenItemModel(
            tmdbId: 1,
            type: 'movie',
            title: 'Dune',
    ...
    test(
      'should remove all seen items for a specific TMDB ID and type',
      () async {
        await dataSource.markAsSeen(tSeenItem);
        await dataSource.markAsSeen(
          SeenItemModel(
    ...
    test(
      'should update poster path for all entries of a specific item',
      () async {
        await dataSource.markAsSeen(
          SeenItemModel(
            tmdbId: 1,
  ...
  group('MediaListLocalDataSource - Notifications', () {
    ...
    test(
      'autoNotify should update release date if item already exists',
      () async {
        final initialDate = DateTime(2023, 10, 1);
        final newDate = DateTime(2023, 10, 2);
