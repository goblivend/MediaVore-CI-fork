...
void main() {
  ...
  group('SettingsProvider - Setters', () {
    test('setDisplayMode should update state and save to prefs', () async {
      ...
      verify(() => mockPrefs.setInt('displayMode', DisplayMode.swipe.index)).called(1);
    });
  ...
  group('SettingsProvider - Palettes', () {
    test('lightPalette should return the palette corresponding to the current index', () async {
      await provider.setLightAppTheme(1); // Parchment
      expect(provider.lightPalette.runtimeType.toString(), contains('Parchment'));
    });

    test('darkPalette should return the palette corresponding to the current index', () async {
